...
impl TextureCache {
    ...
    pub fn get_ref(&self, key: &str) -> Option<&raylib::core::texture::Texture2D> { self.map.get(key) }
    pub fn replace_loaded(&mut self, key: String, tex: raylib::core::texture::Texture2D) { self.map.insert(key, tex); }
}
...
pub fn upload_chunk_mesh(
    ...
    for (mid, mb) in cpu.parts.into_iter() {
        ...
        while q < total_quads {
            ...
            unsafe {
                let pos_start = v_start * 3; let pos_end = pos_start + v_count * 3;
                let norm_start = v_start * 3; let norm_end = norm_start + v_count * 3;
                let uv_start = v_start * 2; let uv_end = uv_start + v_count * 2;
                let col_start = v_start * 4; let col_end = col_start + v_count * 4;
    ...
    Some(ChunkRender { cx: cpu.cx, cz: cpu.cz, bbox: conv::aabb_to_rl(cpu.bbox), parts: parts_gpu, leaf_tint: None })
}
...
impl LeavesShader {
    pub fn load(rl: &mut RaylibHandle, thread: &RaylibThread) -> Option<Self> {
        ...
        let mut s = Self { shader, loc_fog_color, loc_fog_start, loc_fog_end, loc_camera_pos, loc_palette0, loc_palette1, loc_palette2, loc_palette3, loc_strength };
        s.set_autumn_palette([0.905, 0.678, 0.161], [0.847, 0.451, 0.122], [0.710, 0.200, 0.153], [0.431, 0.231, 0.039], 1.0);
        Some(s)
    }
    pub fn set_autumn_palette(&mut self, p0: [f32; 3], p1: [f32; 3], p2: [f32; 3], p3: [f32; 3], strength: f32) {
    ...
    pub fn update_frame_uniforms(&mut self, camera_pos: Vector3, fog_color: [f32; 3], fog_start: f32, fog_end: f32) {
        ...
        if self.loc_camera_pos >= 0 { let cam = [camera_pos.x, camera_pos.y, camera_pos.z]; self.shader.set_shader_value(self.loc_camera_pos, cam); }
    }
}
