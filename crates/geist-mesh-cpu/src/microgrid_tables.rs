    ...
    for y in 0..2 {
        ...
        for z in 0..2 {
            for x in 0..2 {
                ...
                let h = if z == 0 {
                    let mut ok = true;
                    for xi in x..(x + w) { if !grid[1][xi] || used[1][xi] { ok = false; break; } }
                    if ok { 2 } else { 1 }
                } else { 1 };
                for dz in 0..h { for dx in 0..w { used[z + dz][x + dx] = true; } }
                out.push([x as u8, y as u8, z as u8, (x + w) as u8, (y + 1) as u8, (z + h) as u8]);
            }
        }
    }
    out
...
fn gen_rects_for_mask(mask: u8) -> Vec<MicroRect> {
    ...
    for v in 0..2 { for u in 0..2 { grid[v][u] = (mask & (1u8 << ((v << 1) | u))) != 0; } }
    let mut out: Vec<MicroRect> = Vec::new();
    let mut used = [[false; 2]; 2];
    for v in 0..2 {
        for u in 0..2 {
            if !grid[v][u] || used[v][u] { continue; }
            let w = if u == 0 && grid[v][1] && !used[v][1] { 2 } else { 1 };
            let h = if v == 0 {
                let mut ok = true;
                for ui in u..(u + w) { if !grid[1][ui] || used[1][ui] { ok = false; break; } }
                if ok { 2 } else { 1 }
            } else { 1 };
            for dv in 0..h { for du in 0..w { used[v + dv][u + du] = true; } }
...
fn build_boxes_table() -> [Vec<MicroBox>; 256] { std::array::from_fn(|i| gen_boxes_for_occ(i as u8)) }
fn build_rects_table() -> [Vec<MicroRect>; 16] { std::array::from_fn(|i| gen_rects_for_mask(i as u8)) }
