...
impl Bitset {
    fn new(nbits: usize) -> Self { Self { data: vec![0; (nbits + WORD_INDEX_MASK) / BITS_PER_WORD] } }
    #[inline]
    fn set(&mut self, i: usize, v: bool) {
        let w = i >> WORD_INDEX_SHIFT; let b = i & WORD_INDEX_MASK;
        if v { self.data[w] |= 1u64 << b; } else { self.data[w] &= !(1u64 << b); }
    }
    #[inline]
    ...
    #[inline]
    fn clear(&mut self) { self.data.fill(0); }
}
...
struct FaceGrids {
    ...
    kx: Vec<MaterialId>, ky: Vec<MaterialId>, kz: Vec<MaterialId>,
...
impl OccGrids {
    ...
    #[inline] fn idx(&self, ix: usize, iy: usize, iz: usize) -> usize { (ix * self.ny + iy) * self.nz + iz }
    #[inline] fn idx_sx(&self, iy: usize, iz: usize) -> usize { iy * self.nz + iz }
    #[inline] fn idx_sz(&self, ix: usize, iy: usize) -> usize { (iy * self.nx) + ix }
    #[inline] fn occ_get(&self, ix: usize, iy: usize, iz: usize) -> bool { self.occ.get(self.idx(ix, iy, iz)) }
    #[inline] fn occ_set(&mut self, ix: usize, iy: usize, iz: usize, v: bool) { let i = self.idx(ix, iy, iz); self.occ.set(i, v); }
}
...
impl<'a> ParityMesher<'a> {
    pub fn new(
        ...
        let nx = s * sx; let ny = s * sy; let nz = s * sz;
    ...
    fn world_block(&self, nx: i32, ny: i32, nz: i32) -> Block {
        if let Some(ed) = self.edits { ed.get(&(nx, ny, nz)).copied().unwrap_or_else(|| self.world.block_at_runtime(self.reg, nx, ny, nz)) }
        else { self.world.block_at_runtime(self.reg, nx, ny, nz) }
    }

    pub fn build_occupancy(&mut self) {
        let s = self.s; let (sx, sy, sz) = (self.sx, self.sy, self.sz);
        #[inline] fn occ_bit_s2(occ: u8, mx: usize, my: usize, mz: usize) -> bool { let i = ((my & 1) << 2) | ((mz & 1) << 1) | (mx & 1); ((occ >> i) & 1) != 0 }
                ...
                if let Some(occ) = ty.variant(b.state).occupancy {
                    ...
                    for mz in 0..s { for my in 0..s { for mx in 0..s {
                        if occ_bit_s2(occ, mx, my, mz) {
                            let ix = x * s + mx; let iy = y * s + my; let iz = z * s + mz;
        ...
        for ly in 0..sy { for lz in 0..sz {
            ...
            if let Some(ty) = self.reg.get(nb.id) {
                ...
                if ty.is_solid(nb.state) && matches!(ty.shape, geist_blocks::types::Shape::Cube | geist_blocks::types::Shape::AxisCube { .. }) {
                    ...
                    let y0 = ly * s; let z0 = lz * s;
                    for mz in 0..s { for my in 0..s {
                        if occ_bit_s2(occ, 1, my, mz) {
                            let iy = y0 + my; let iz = z0 + mz; let i = self.occs.idx_sx(iy, iz);
                            self.occs.seam_x.set(i, true);
                        }
                    }}
        ...
        for ly in 0..sy { for lx in 0..sx {
            ...
            if let Some(ty) = self.reg.get(nb.id) {
                if ty.is_solid(nb.state) && matches!(ty.shape, geist_blocks::types::Shape::Cube | geist_blocks::types::Shape::AxisCube { .. }) {
                    ...
                    let x0 = lx * s; let y0 = ly * s;
                    for my in 0..s { for mx in 0..s {
                        if occ_bit_s2(occ, mx, my, 1) {
                            let ix = x0 + mx; let iy = y0 + my; let i = self.occs.idx_sz(ix, iy);
    ...
    pub fn compute_parity_and_materials(&mut self) {
        let s = self.s; let (nx, ny, nz) = (self.occs.nx, self.occs.ny, self.occs.nz);
        // X faces
        let t0 = Instant::now();
        for ix in 0..=nx { // include +X boundary
            for iy in 0..ny { for iz in 0..nz {
                let a = if ix == 0 { self.occs.seam_x.get(self.occs.idx_sx(iy, iz)) } else { self.occs.occ_get(ix - 1, iy, iz) };
                let b = if ix == nx { false } else { self.occs.occ_get(ix, iy, iz) };
                let p = a ^ b;
                let idx = self.grids.idx_x(ix, iy, iz);
                self.grids.px.set(idx, p);
                if !p { self.grids.kx[idx] = MaterialId(0); continue; }
        ...
        for iy in 0..=ny { // include top boundary
            for iz in 0..nz { for ix in 0..nx {
                let a = if iy == 0 { false } else { self.occs.occ_get(ix, iy - 1, iz) };
                let b = if iy == ny { false } else { self.occs.occ_get(ix, iy, iz) };
                let p = a ^ b;
                let idx = self.grids.idx_y(ix, iy, iz);
                self.grids.py.set(idx, p);
                if !p { self.grids.ky[idx] = MaterialId(0); continue; }
                // a==1 => +Y face of below cell; b==1 => -Y face of above cell
                let owner_pos = a; self.grids.oy.set(idx, owner_pos);
        ...
        for iz in 0..=nz {
            for iy in 0..ny { for ix in 0..nx {
                let a = if iz == 0 { self.occs.seam_z.get(self.occs.idx_sz(ix, iy)) } else { self.occs.occ_get(ix, iy, iz - 1) };
                let b = if iz == nz { false } else { self.occs.occ_get(ix, iy, iz) };
                let p = a ^ b;
                let idx = self.grids.idx_z(ix, iy, iz);
                self.grids.pz.set(idx, p);
                if !p { self.grids.kz[idx] = MaterialId(0); continue; }
                // a==1 => +Z face of back cell; b==1 => -Z face of front cell
                let owner_pos = a; self.grids.oz.set(idx, owner_pos);
