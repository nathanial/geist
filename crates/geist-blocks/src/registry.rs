...
impl Facing {
    ...
    fn from_str(s: &str) -> Facing {
        match s { "north" => Facing::North, "south" => Facing::South, "west" => Facing::West, "east" => Facing::East, _ => Facing::North }
    }
}
    ...
    pub fn material_for(&self, role: FaceRole, state: BlockState, ty: &BlockType) -> Option<MaterialId> {
        ...
        match pick {
            ...
            ResolvedSelector::By { by, map } => {
                if let Some(val) = ty.state_prop_value(state, by) { map.get(val).copied() } else { None }
            }
        }
    }
}
...
impl BlockRegistry {
    pub fn new() -> Self {
        Self { materials: MaterialCatalog::new(), blocks: Vec::new(), by_name: HashMap::new(), unknown_block_id: None }
    }

    #[inline]
    pub fn get(&self, id: BlockId) -> Option<&BlockType> { self.blocks.get(id as usize) }

    pub fn id_by_name(&self, name: &str) -> Option<BlockId> { self.by_name.get(name).copied() }
    ...
    pub fn from_configs(materials: MaterialCatalog, cfg: BlocksConfig) -> Result<Self, Box<dyn Error>> {
        ...
        let profiles: HashMap<String, LightProfile> = cfg.lighting.as_ref().map(|l| l.profiles.clone()).unwrap_or_default();
        for def in cfg.blocks.into_iter() {
            ...
            let light: CompiledLight = match def.light.or_else(|| def.light_profile.as_ref().and_then(|n| profiles.get(n).cloned())) {
                Some(LightProfile::Omni { attenuation, max_range }) => CompiledLight::Omni { attenuation, max_range },
            ...
            let (pre_top, pre_bottom, pre_side, pre_occ, pre_vars) = {
                ...
                let states_len: usize = if total_bits == 0 { 1 } else { 1usize << total_bits.min(16) };
                ...
                let fill_role = |role: FaceRole| -> Vec<MaterialId> {
                    ...
                    for s in 0..states_len {
                        ...
                        let id = ty.materials.material_for(role, state, &ty).unwrap_or(unknown_mid);
                        v.push(id);
                    }
                    v
                };
                        ...
                        Shape::Slab { half_from } | Shape::Stairs { half_from, .. } => {
                            ...
                            let occ8 = match &ty.shape {
                                ...
                                Shape::Stairs { facing_from, .. } => {
                                    let facing = Facing::from_str(ty.state_prop_value(state, facing_from).unwrap_or("north"));
                                    Some(occ_stairs(facing, is_top))
                                }
                                _ => None,
                            };
                            (occ_mask, ShapeVariant { occupancy: occ8, dynamic: None })
                        }
                        Shape::Pane => (0, ShapeVariant { occupancy: None, dynamic: Some(DynamicShape::Pane) }),
                        Shape::Fence => (0, ShapeVariant { occupancy: None, dynamic: Some(DynamicShape::Fence) }),
                        Shape::Gate { .. } => (0, ShapeVariant { occupancy: None, dynamic: Some(DynamicShape::Gate) }),
                        Shape::Carpet => (0, ShapeVariant { occupancy: None, dynamic: Some(DynamicShape::Carpet) }),
                        _ => {
                            if ty.is_solid(state) {
                                (0b11_1111, ShapeVariant { occupancy: None, dynamic: None })
                            } else {
                                (0, ShapeVariant { occupancy: None, dynamic: None })
                            }
                        }
                    };
                    occ.push(m);
                ...
                (fill_role(FaceRole::Top), fill_role(FaceRole::Bottom), fill_role(FaceRole::Side), occ, vars)
            };
        ...
        if let Some(name) = unknown_name { reg.unknown_block_id = reg.id_by_name(&name); }
        reg.by_name = reg.blocks.iter().map(|t| (t.name.clone(), t.id)).collect();
        Ok(reg)
    }

    pub fn unknown_block_id_or_panic(&self) -> BlockId {
        if let Some(id) = self.unknown_block_id.or_else(|| self.id_by_name("unknown")) { id } else { panic!("Unknown block fallback not configured. Set `unknown_block = \"<name>\"` in blocks.toml and define that block.") }
    }

    pub fn make_block_by_name(&self, name: &str, props: Option<&std::collections::HashMap<String, String>>) -> Option<Block> {
        let id = self.id_by_name(name)?;
        let state = if let Some(p) = props { self.get(id).map(|ty| ty.pack_state(p)).unwrap_or(0) } else { 0 };
        Some(Block { id, state })
    }
}
...
#[derive(Clone, Debug)]
pub struct StateField { pub name: String, pub values: Vec<String>, pub bits: u32, pub offset: u32 }

#[derive(Clone, Debug, Default)]
pub struct ShapeVariant { pub occupancy: Option<u8>, pub dynamic: Option<DynamicShape> }

#[derive(Copy, Clone, Debug)]
pub enum DynamicShape { Pane, Fence, Gate, Carpet }

#[derive(Clone, Copy, Debug, Default)]
pub struct SeamPolicy { pub dont_occlude_same: bool, pub dont_project_fixups: bool }
...
fn compile_shape(shape: Option<ShapeConfig>) -> Shape {
    ...
    match shape.unwrap_or(Simple("cube".into())) {
        Simple(k) => match k.as_str() {
            "cube" => Shape::Cube,
            "slab" => Shape::Slab { half_from: "half".into() },
            "stairs" => Shape::Stairs { facing_from: "facing".into(), half_from: "half".into() },
            "pane" => Shape::Pane,
            "fence" => Shape::Fence,
            "gate" => Shape::Gate { facing_from: "facing".into(), open_from: "open".into() },
            "carpet" => Shape::Carpet,
            _ => Shape::None,
        },
        Detailed(ShapeDetailed { kind, axis, half, facing, open }) => match kind.as_str() {
            "cube" => Shape::Cube,
            "axis_cube" => Shape::AxisCube { axis_from: axis.map(|p: PropertyFrom| p.from).unwrap_or_else(|| "axis".to_string()) },
            "slab" => Shape::Slab { half_from: half.map(|p| p.from).unwrap_or_else(|| "half".to_string()) },
            "stairs" => Shape::Stairs { facing_from: facing.map(|p| p.from).unwrap_or_else(|| "facing".to_string()), half_from: half.map(|p| p.from).unwrap_or_else(|| "half".to_string()) },
            "pane" => Shape::Pane,
            "fence" => Shape::Fence,
            "gate" => Shape::Gate { facing_from: facing.map(|p| p.from).unwrap_or_else(|| "facing".to_string()), open_from: open.map(|p| p.from).unwrap_or_else(|| "open".to_string()) },
            "carpet" => Shape::Carpet,
            _ => Shape::None,
        },
    }
...
#[inline]
fn bit2(x: usize, y: usize, z: usize) -> u8 { 1u8 << (((y & 1) << 2) | ((z & 1) << 1) | (x & 1)) }

#[inline]
fn occ_slab(is_top: bool) -> u8 { let y = if is_top { 1 } else { 0 }; bit2(0, y, 0) | bit2(1, y, 0) | bit2(0, y, 1) | bit2(1, y, 1) }
    ...
    fn resolve_selector(matcat: &MaterialCatalog, sel: &MaterialSelector) -> Option<ResolvedSelector> {
        match sel {
            ...
            MaterialSelector::By { by, map } => {
                ...
                for (k, v) in map.iter() { if let Some(id) = matcat.get_id(v) { out.insert(k.clone(), id); } }
...
fn compute_state_layout(schema: &HashMap<String, Vec<String>>) -> (Vec<StateField>, HashMap<String, usize>) {
    ...
    for (i, f) in fields.iter().enumerate() { index.insert(f.name.clone(), i); }
    (fields, index)
}
...
impl BlockType {
    ...
    pub fn light_is_beam(&self) -> bool { matches!(self.light, CompiledLight::Beam { .. }) }
    pub fn omni_attenuation(&self) -> u8 { match self.light { CompiledLight::Omni { attenuation, .. } => attenuation, _ => 32 } }
    ...
    pub fn state_prop_value<'a>(&'a self, state: BlockState, prop: &str) -> Option<&'a str> {
        if self.state_fields.is_empty() { return None; }
        let &i = self.prop_index.get(prop)?;
        let f = &self.state_fields[i];
        if f.bits == 0 { return f.values.first().map(|s| s.as_str()); }
    ...
    pub fn state_prop_is_value(&self, state: BlockState, prop: &str, expect: &str) -> bool { self.state_prop_value(state, prop) == Some(expect) }
    pub fn pack_state(&self, props: &std::collections::HashMap<String, String>) -> BlockState {
        if self.state_fields.is_empty() { return 0; }
        let mut acc: u32 = 0;
        for f in &self.state_fields {
            if f.bits == 0 { continue; }
            let sel_idx: u32 = match props.get(&f.name) { Some(val) => f.values.iter().position(|s| s == val).unwrap_or(0) as u32, None => 0 };
    ...
    pub fn material_for_cached(&self, role: FaceRole, state: BlockState) -> MaterialId {
        match role {
            FaceRole::Top => { let len = self.pre_mat_top.len(); self.pre_mat_top[state as usize & (len - 1)] }
            FaceRole::Bottom => { let len = self.pre_mat_bottom.len(); self.pre_mat_bottom[state as usize & (len - 1)] }
            FaceRole::Side | FaceRole::All => { let len = self.pre_mat_side.len(); self.pre_mat_side[state as usize & (len - 1)] }
        }
    }
    #[inline]
    pub fn occlusion_mask_cached(&self, state: BlockState) -> u8 { let len = self.pre_occ_mask.len(); self.pre_occ_mask[state as usize & (len - 1)] }
    #[inline]
    pub fn variant(&self, state: BlockState) -> &ShapeVariant { let len = self.pre_shape_variants.len(); &self.pre_shape_variants[state as usize & (len - 1)] }
}
