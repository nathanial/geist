...
fn pack_state_roundtrip_fixed() {
    ...
    let def = BlockDef { name: "t".into(), id: Some(0), solid: Some(true), blocks_skylight: Some(true), propagates_light: Some(false), emission: Some(0), light_profile: None, light: None, shape: None, materials: None, state_schema: Some(schema.clone()), seam: None };
    let cfg = BlocksConfig { blocks: vec![def], lighting: None, unknown_block: None };
...
fn material_cache_matches_dynamic_fixed() {
    ...
    let materials = MaterialCatalog::from_toml_str(r#"
        [materials]
        red = ["assets/blocks/red.png"]
        blue = ["assets/blocks/blue.png"]
        unknown = ["assets/blocks/unknown.png"]
    "#).unwrap();
    let schema = HashMap::from([("material".to_string(), vec!["red".to_string(), "blue".to_string()])]);
    let materials_def = MaterialsDef { all: None, top: None, bottom: None, side: Some(MaterialSelector::By { by: "material".into(), map: HashMap::from([("red".into(), "red".into()), ("blue".into(), "blue".into())]) }) };
    let def = BlockDef { name: "painted".into(), id: Some(1), solid: Some(true), blocks_skylight: Some(true), propagates_light: Some(false), emission: Some(0), light_profile: None, light: None, shape: Some(ShapeConfig::Simple("cube".into())), materials: Some(materials_def), state_schema: Some(schema.clone()), seam: None };
    let cfg = BlocksConfig { blocks: vec![def], lighting: None, unknown_block: Some("unknown".into()) };
    let reg = BlockRegistry::from_configs(materials, cfg).expect("registry");
    ...
    let dyn_red = ty.materials.material_for(FaceRole::Side, red_state, ty).unwrap();
    let dyn_blue = ty.materials.material_for(FaceRole::Side, blue_state, ty).unwrap();
...
fn slab_occlusion_and_occupancy_half_fixed() {
    let materials = MaterialCatalog::from_toml_str(r#"
        ...
        unknown = ["assets/blocks/unknown.png"]
    "#).unwrap();
    let schema = HashMap::from([("half".to_string(), vec!["bottom".to_string(), "top".to_string()])]);
    let def = BlockDef { name: "slab".into(), id: Some(2), solid: Some(true), blocks_skylight: Some(false), propagates_light: Some(true), emission: Some(0), light_profile: None, light: None, shape: Some(ShapeConfig::Simple("slab".into())), materials: None, state_schema: Some(schema.clone()), seam: None };
    let cfg = BlocksConfig { blocks: vec![def], lighting: None, unknown_block: Some("unknown".into()) };
    let reg = BlockRegistry::from_configs(materials, cfg).expect("registry");
