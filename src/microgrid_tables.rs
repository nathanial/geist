...
fn gen_boxes_for_occ(occ: u8) -> Vec<MicroBox> {
    ...
    // For each y-layer, merge up to 2x2 rectangles; each becomes a box of height 1 half-step.
...
fn gen_rects_for_mask(mask: u8) -> Vec<MicroRect> {
    // Boundary emptiness greedy merge on a 2x2 (u,v) grid.
    // Bit layout: bit = (v<<1) | u.
