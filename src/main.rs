...
use mesher::{
    ChunkMeshCPU, FaceMaterial, NeighborsLoaded, TextureCache, build_chunk_greedy_cpu_buf,
    upload_chunk_mesh,
};
use raylib::prelude::*;
...
fn main() {
    ...
    let mut walker =
        player::Walker::new(Vector3::new(spawn.x, (chunk_size_y as f32) * 0.8, spawn.z));
    ...
    for _ in 0..worker_count {
        ...
        thread::spawn(move || {
            while let Ok(job) = wrx.recv() {
                ...
                let snap_map: std::collections::HashMap<(i32, i32, i32), voxel::Block> =
                    snap_vec.into_iter().collect();
                if let Some(cpu) = build_chunk_greedy_cpu_buf(
                    &buf,
                    Some(&ls),
                    &w,
                    Some(&snap_map),
    ...
    while !rl.window_should_close() {
        ...
        if walk_mode {
            cam.update_look_only(&mut rl, dt);
        } else {
        ...
        if rl.is_key_pressed(KeyboardKey::KEY_ONE) {
            place_type = voxel::Block::Dirt;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_TWO) {
            place_type = voxel::Block::Stone;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_THREE) {
            place_type = voxel::Block::Sand;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_FOUR) {
            place_type = voxel::Block::Grass;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_FIVE) {
            place_type = voxel::Block::Snow;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_SIX) {
            place_type = voxel::Block::Glowstone;
        }
        if rl.is_key_pressed(KeyboardKey::KEY_SEVEN) {
            place_type = voxel::Block::Beacon;
        }
        ...
        for out in res_rx.try_iter() {
            ...
            if borders_changed {
                for nk in neighbors.iter() {
                    if !loaded.contains_key(nk) || pending.contains(nk) {
                        continue;
                    }
                    let (cx, cz) = *nk;
                    let nmask = NeighborsLoaded {
        ...
        // Handle block edits (remove/place) via mouse buttons
        {
            ...
            let sampler = |wx: i32, wy: i32, wz: i32| -> voxel::Block {
                if let Some(b) = edit_store.get(wx, wy, wz) {
                    return b;
                }
                let cx = wx.div_euclid(sx);
                let cz = wz.div_euclid(sz);
                if let Some(buf) = loaded_bufs.get(&(cx, cz)) {
                    buf.get_world(wx, wy, wz).unwrap_or(voxel::Block::Air)
                } else {
                    world.block_at(wx, wy, wz)
                }
            };
            let is_solid = |wx: i32, wy: i32, wz: i32| -> bool { sampler(wx, wy, wz).is_solid() };
            let ray_hit = rl.is_mouse_button_pressed(MouseButton::MOUSE_BUTTON_LEFT)
                        ...
                        if wy >= 0 && wy < chunk_size_y as i32 {
                            ...
                            if place_type.emission() > 0 {
                                if matches!(place_type, voxel::Block::Beacon) {
                                    lighting_store.add_beacon_world(
                                        wx,
                                        wy,
                                        wz,
                                        place_type.emission(),
                                    ...
                                    lighting_store.add_emitter_world(
                                        wx,
                                        wy,
                                        wz,
                                        place_type.emission(),
...

        {
            ...
            // Placement/removal preview: face outline on targeted voxel face
            {
                ...
                if let Some(hit) =
                    raycast::raycast_first_hit_with_face(org, dir, max_dist, is_solid)
                {
                    let eps = 0.01_f32;
                    let c = Color::WHITE;
                    if hit.nx != 0 {
                        let xp = (hit.bx as f32) + if hit.nx > 0 { 1.0 + eps } else { -eps };
                        let y0 = hit.by as f32;
                        let y1 = y0 + 1.0;
                        let z0 = hit.bz as f32;
                        let z1 = z0 + 1.0;
                        let p1 = Vector3::new(xp, y0, z0);
                        let p2 = Vector3::new(xp, y1, z0);
                        let p3 = Vector3::new(xp, y1, z1);
                        let p4 = Vector3::new(xp, y0, z1);
                        d3.draw_line_3D(p1, p2, c);
                        d3.draw_line_3D(p2, p3, c);
                        d3.draw_line_3D(p3, p4, c);
                        d3.draw_line_3D(p4, p1, c);
                    } else if hit.ny != 0 {
                        let yp = (hit.by as f32) + if hit.ny > 0 { 1.0 + eps } else { -eps };
                        let x0 = hit.bx as f32;
                        let x1 = x0 + 1.0;
                        let z0 = hit.bz as f32;
                        let z1 = z0 + 1.0;
                        let p1 = Vector3::new(x0, yp, z0);
                        let p2 = Vector3::new(x1, yp, z0);
                        let p3 = Vector3::new(x1, yp, z1);
                        let p4 = Vector3::new(x0, yp, z1);
                        d3.draw_line_3D(p1, p2, c);
                        d3.draw_line_3D(p2, p3, c);
                        d3.draw_line_3D(p3, p4, c);
                        d3.draw_line_3D(p4, p1, c);
                        ...
                        let y0 = hit.by as f32;
                        let y1 = y0 + 1.0;
                        let p1 = Vector3::new(x0, y0, zp);
                        let p2 = Vector3::new(x1, y0, zp);
                        let p3 = Vector3::new(x1, y1, zp);
                        let p4 = Vector3::new(x0, y1, zp);
                        d3.draw_line_3D(p1, p2, c);
                        d3.draw_line_3D(p2, p3, c);
                        d3.draw_line_3D(p3, p4, c);
                        d3.draw_line_3D(p4, p1, c);
        ...
        // Debug overlay: current chunk coordinates and camera facing
        {
            ...
            let mut yawn = cam.yaw % 360.0;
            if yawn < 0.0 {
                yawn += 360.0;
            }
            let dir = if yawn >= 315.0 || yawn < 45.0 {
                "E"
            } else if yawn < 135.0 {
                "S"
            } else if yawn < 225.0 {
            ...
            let line1 = format!(
                "Chunk: ({}, {}), Pos: ({:.1}, {:.1}, {:.1})",
                ccx_dbg, ccz_dbg, cam.position.x, cam.position.y, cam.position.z
            );
            let line2 = format!(
                "Facing: {}  yaw={:.1}°  fwd=({:.2},{:.2},{:.2})",
                dir, yawn, f.x, f.y, f.z
            );
            ...
            let line3 = format!(
                "Local: ({:.2}, {:.2})  edge_dx={:.2} edge_dz={:.2}",
                lx, lz, dx_edge, dz_edge
            );
        ...
        let hud = format!(
            "{}: Tab capture, WASD{} move{}, V toggle mode, F wireframe, G grid, B bounds, L add light, K remove light | Place: {:?} (1-7 to select) | LMB remove, RMB place",
            hud_mode,
            if walk_mode { "" } else { "+QE" },
            if walk_mode {
