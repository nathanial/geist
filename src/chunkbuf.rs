...
impl ChunkBuf {
    #[inline]
    pub fn idx(&self, x: usize, y: usize, z: usize) -> usize { (y * self.sz + z) * self.sx + x }

    #[inline]
    pub fn get_local(&self, x: usize, y: usize, z: usize) -> Block { self.blocks[self.idx(x,y,z)] }

    #[inline]
    pub fn contains_world(&self, wx: i32, wy: i32, wz: i32) -> bool {
        if wy < 0 || wy >= self.sy as i32 { return false; }
    ...
    pub fn get_world(&self, wx: i32, wy: i32, wz: i32) -> Option<Block> {
        if !self.contains_world(wx, wy, wz) { return None; }
        if wy < 0 || wy >= self.sy as i32 { return Some(Block::Air); }
        let x0 = self.cx * self.sx as i32;
        let z0 = self.cz * self.sz as i32;
...
pub fn generate_chunk_buffer(world: &World, cx: i32, cz: i32) -> ChunkBuf {
    ...
    let x0 = cx * sx as i32; let z0 = cz * sz as i32;
    for z in 0..sz { for y in 0..sy { for x in 0..sx {
        let wx = x0 + x as i32; let wy = y as i32; let wz = z0 + z as i32;
