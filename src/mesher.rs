...
fn is_occluder(
    ...
    if buf.contains_world(nx, ny, nz) {
        ...
        if ny < 0 || ny >= buf.sy as i32 { return false; }
    ...
    if nx < x0 { neighbor_loaded = nmask.neg_x; }
    else if nx >= x1 { neighbor_loaded = nmask.pos_x; }
    else if nz < z0 { neighbor_loaded = nmask.neg_z; }
