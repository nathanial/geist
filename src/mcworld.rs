...
fn parse_mcstructure_le(reader: &mut dyn std::io::Read) -> Result<McStructure, String> {
    ...
    let root = match nbt { bnbt::Value::Compound(c) => c, _ => return Err("root not a compound".into()) };
    // size: int array [x,y,z]
    let size = match root.get("size") {
        Some(bnbt::Value::List(xs)) => {
            let mut arr = [0i32; 3];
            for (i, it) in xs.iter().take(3).enumerate() {
                arr[i] = match it { bnbt::Value::Int(v) => *v, _ => 0 };
            }
            (arr[0], arr[1], arr[2])
        }
        _ => (0, 0, 0),
    };
    if size.0 <= 0 || size.1 <= 0 || size.2 <= 0 { return Err("invalid size in mcstructure".into()); }
                ...
                if let Some(bnbt::Value::List(bis)) = p0.get("block_indices") {
                    ...
                    for part in bis {
                        if let bnbt::Value::List(li) = part {
                            for v in li {
                                if let bnbt::Value::Int(ix) = v { indices.push((*ix) as usize); }
                            }
                        }
                    }
                }
    ...
    if indices.len() != expect {
        ...
        if indices.len() < expect { indices.resize(expect, 0); }
        else { indices.truncate(expect); }
    }
    Ok(McStructure { size, palette, indices })
}

#[cfg(feature = "mcworld")]
pub fn load_mcworlds_in_dir(dir: &Path, base_y: i32, edits: &mut crate::edit::EditStore) -> Result<Vec<(String, (i32,i32,i32), (i32,i32,i32))>, String> {
    let mut out = Vec::new();
    for ent in std::fs::read_dir(dir).map_err(|e| format!("read_dir {:?}: {}", dir, e))? {
        let ent = ent.map_err(|e| format!("read_dir entry: {}", e))?;
        let p = ent.path();
        if !p.is_file() { continue; }
        if p.extension().map(|e| e.to_string_lossy().to_lowercase()) != Some("mcworld".to_string()) { continue; }
        ...
        for i in 0..zip.len() {
            let name = zip.by_index(i).map_err(|e| e.to_string())?.name().to_string().replace('\\', "/");
            if !name.to_lowercase().ends_with(".mcstructure") { continue; }
            if !name.contains("structures/") { continue; }
            ...
            match parse_mcstructure_le(&mut cur) {
                Ok(ms) => {
                    ...
                    for y in 0..sy {
                        for z in 0..sz {
                            for x in 0..sx {
                                let i = (y as usize) * (sz as usize) * (sx as usize) + (z as usize) * (sx as usize) + (x as usize);
                                let b = blocks[i];
                    ...
                    log::info!("Imported mcworld {:?} structure {} ({}x{}x{}) blocks={} at y={}.", p, name, sx, sy, sz, placed, base_y);
                    out.push((format!("{}!{}", p.display(), name), (0, base_y, 0), (sx, sy, sz)));
                }
                Err(e) => {
